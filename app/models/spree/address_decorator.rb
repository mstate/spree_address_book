...
Spree::Address.class_eval do
  ...
  # can modify an address if it's not been used for a shipped shipment
  def editable?
    new_record? || shipments.shipped.empty? # && Spree::Order.complete.where("bill_address_id = ? OR ship_address_id = ?", self.id, self.id).count == 0)
  end
